...
type ServerConfig struct {
	...
	// TLSCertFile is the path to TLS certificate file (optional).
	// If set, server will use HTTPS.
	TLSCertFile string `mapstructure:"tls_cert_file"`

	// TLSKeyFile is the path to TLS private key file (optional).
...
type AgentConfig struct {
	...
	// Optional: If empty, a unique ID will be auto-generated by the Agent worker
	// based on hostname and timestamp.
...
func (c *Config) Validate() error {
	...
	if c.Server.TLSCertFile != "" && c.Server.TLSKeyFile == "" {
		return fmt.Errorf("server.tls_key_file is required when tls_cert_file is set\n" +
			"  Set WATERFLOW_SERVER_TLS_KEY_FILE=/path/to/key.pem or update server.tls_key_file in config file")
	}
	if c.Server.TLSKeyFile != "" && c.Server.TLSCertFile == "" {
		return fmt.Errorf("server.tls_cert_file is required when tls_key_file is set\n" +
			"  Set WATERFLOW_SERVER_TLS_CERT_FILE=/path/to/cert.pem or update server.tls_cert_file in config file")
	}
