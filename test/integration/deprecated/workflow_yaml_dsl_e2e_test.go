...
`
	...
	serverURL := getEnvOrDefault("SERVER_URL", "http://localhost:18080")
	ctx, cancel := context.WithTimeout(context.Background(), 90*time.Second)
	defer cancel()

	// WHEN: Submit workflow via API
	submitURL := fmt.Sprintf("%s/api/v1/workflows", serverURL)
	resp, err := http.Post(submitURL, "application/x-yaml", io.NopCloser([]byte(complexWorkflow)))
...
`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			resp, err := http.Post(submitURL, "application/x-yaml", io.NopCloser([]byte(tc.yaml)))
...
`
		...
		resp, err := http.Post(submitURL, "application/x-yaml", io.NopCloser([]byte(circularWorkflow)))
		require.NoError(t, err)
		defer resp.Body.Close()
...
`
		...
		resp, err := http.Post(submitURL, "application/x-yaml", io.NopCloser([]byte(invalidRefWorkflow)))
		require.NoError(t, err)
		defer resp.Body.Close()
...
`
		...
		resp, err := http.Post(submitURL, "application/x-yaml", io.NopCloser([]byte(parallelWorkflow)))
		require.NoError(t, err)
		defer resp.Body.Close()
...
func getEnvOrDefault(key, defaultValue string) string {
	if value := testutil.GetEnv(key, ""); value != "" {
		return value
	}
	return defaultValue
}
